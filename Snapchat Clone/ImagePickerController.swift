...
class ImagePickerController: UIViewController {
    ...
    func selectImage(_ image: UIImage) {
        ...
        selectedImage = image
        performSegue(withIdentifier: "displayImage", sender: image)
    }
    
    /// TODO: override prepareForSegue to pass the selected image over to the next view
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let viewControllerTo = segue.destination as? ImagePreviewViewController {
            viewControllerTo.selectedImage = selectedImage
        }
    }
